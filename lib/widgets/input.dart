...
// URL entry with a per-subscription Happ toggle: default is an honest fetch;
// opt in to the Happ client for panels that gate their full node set behind it.
class HappUrlDialog extends StatefulWidget {
  final String title;
  final String? labelText;
  final String? hintText;
  final String initialUrl;
  final bool initialHapp;
  final FormFieldValidator<String>? validator;
  ...
  const HappUrlDialog({
    ...
    this.hintText,
    this.initialUrl = '',
    this.initialHapp = false,
    this.validator,
  });

  @override
  State<HappUrlDialog> createState() => _HappUrlDialogState();
}

class _HappUrlDialogState extends State<HappUrlDialog> {
  final _formKey = GlobalKey<FormState>();
  ...
  void _submit() {
    if (_formKey.currentState?.validate() == false) return;
    Navigator.of(context).pop((url: _controller.text.trim(), happ: _happ));
  }
