...
void main() {
  ...
  Map<String, dynamic> baseConfig() => {
    ...
    'rules': <dynamic>['IN-TYPE,INNER,DIRECT', 'MATCH,DIRECT'],
  };
  ...
  test(
    'master ON manual: only byedpi-local proxy injected, no rules added',
    () {
      final cfg = baseConfig();
      injectByeDpi(
        rawConfig: cfg,
        settings: const ByeDpiSettings(enabled: true, mode: ByeDpiMode.manual),
        hosts: hosts,
      );
      final proxies = cfg['proxies'] as List;
      expect(
        proxies.any((p) => p is Map && p['name'] == 'byedpi-local'),
        isTrue,
      );
      final rules = cfg['rules'] as List;
      expect(
        rules.whereType<String>().any((r) => r.startsWith('DOMAIN-SUFFIX')),
        isFalse,
      );
      expect(
        cfg.containsKey('proxy-groups') &&
            (cfg['proxy-groups'] as List).any(
  ...
  test(
    'master ON auto fallback OFF: proxy + DOMAIN-SUFFIX rules pointing to byedpi-local',
    () {
      final cfg = baseConfig();
      injectByeDpi(
        rawConfig: cfg,
      ...
      for (final h in hosts) {
        expect(
          domainRules.any((r) => r.contains(h) && r.endsWith(',byedpi-local')),
          isTrue,
        );
      }
      final groups = cfg['proxy-groups'] as List;
      expect(
        groups.any((g) => g is Map && g['name'] == 'byedpi-fallback'),
        isFalse,
      );
    },
  );
  ...
  test(
    'master ON auto fallback ON: proxy + fallback group + DOMAIN-SUFFIX rules pointing to byedpi-fallback',
    () {
      final cfg = baseConfig();
      injectByeDpi(
        rawConfig: cfg,
      ...
      expect(
        fallbackGroup['proxies'] as List,
        containsAll(['byedpi-local', 'VPN']),
      );
      ...
      for (final h in hosts) {
        expect(
          domainRules.any(
            (r) => r.contains(h) && r.endsWith(',byedpi-fallback'),
          ),
          isTrue,
        );
      }
